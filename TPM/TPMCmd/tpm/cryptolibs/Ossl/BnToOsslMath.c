...
#include "TPM/TPMCmd/tpm/cryptolibs/Ossl/include/BnOssl.h"

#ifdef MATH_LIB_OSSL
#include "TPM/TPMCmd/tpm/cryptolibs/Ossl/include/Ossl/BnToOsslMath_fp.h"
...
BOOL OsslToTpmBn(bigNum bn, BIGNUM* osslBn)
{
    ...
    if(bn != NULL)
    {
        ...
        for(i = 0; i < osslBn->top; i++)
            bn->d[i] = osslBn->d[i];
        BnSetTop(bn, (unsigned int)osslBn->top);
    }
    return TRUE;
Error:
    return FALSE;
