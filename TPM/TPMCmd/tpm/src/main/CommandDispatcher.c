...
#include "TPM/TPMCmd/tpm/include/private/CommandDispatchData.h"
...
#include "TPM/TPMCmd/tpm/include/private/Commands.h"
...
#  else
        //LOG_INFO("dtype is %d HANDLE %d type %d", dType, HANDLE_FIRST_FLAG_TYPE, type);
...
#  endif
    ...
    *handleCount                    = 0;
    switch(command->code)
    {
#include "TPM/TPMCmd/tpm/include/private/HandleProcess.h"
#  undef handles
        default:
...
CommandDispatcher(COMMAND* command)
{
    ...
    {
#include "TPM/TPMCmd/tpm/include/private/CommandDispatcher.h"
...
Exit:
    ...
    desc = s_CommandDataArray[command->index];
    #ifdef DEBUGS
    LOG_INFO("Index: %d, desc %d", command->index, desc);
    #endif
...
#  else
        if(dType < PARAMETER_FIRST_FLAG_TYPE)
        {
            ...
            result            = f(&commandIn[offset],
                       ...
                       &command->parameterSize);
            if (result != TPM_RC_SUCCESS){
                LOG_INFO("Dtype: %d PARAM: %d ", dType, PARAMETER_FIRST_FLAG_TYPE);
                LOG_INFO("Result %d", result);
            }
        }
        else
