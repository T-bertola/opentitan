...
#include "TPM/TPMCmd/tpm/include/public/ACT.h"
...
)
{
    ...
    if(!MemoryEqual2B(&session->u2.policyDigest.b, &authPolicy.b)){
        for(int i =0; i < session->u2.policyDigest.b.size; i ++){
            LOG_INFO("Policy: %04x Other: %04x",session->u2.policyDigest.b.buffer[i], authPolicy.b.buffer[i] );
        }
...
)
{
    ...
    if(lockoutAuthCheck)
    {
        if(gp.lockOutAuthEnabled == FALSE){
            LOG_INFO("gp.lockOutAuthEnabled");
    ...
    else
    {
        ...
        if(gp.failedTries >= gp.maxTries){
            LOG_INFO("gp.failedTries");
            //FOR NOW BECAUSE DA IS OUT OF SCOPE, BYPASS THIS FOR NOW
            return TPM_RC_SUCCESS;
            //return TPM_RC_LOCKOUT;
        }
#if USE_DA_USED
    ...
    for(sessionIndex = 0; sessionIndex < command->sessionNum; sessionIndex++)
    {
        ...
        if(s_sessionHandles[sessionIndex] == TPM_RS_PW)
        {
            if(s_associatedHandles[sessionIndex] == TPM_RH_UNASSIGNED){
                LOG_INFO("Error Return Result: %08x",TPM_RCS_ATTRIBUTES + errorIndex );
                return TPM_RCS_HANDLE + errorIndex;
            }
        ...
        else
        {
            ...
            if(session->attributes.isTrialPolicy == SET){
                LOG_INFO("Parse Result %d", TPM_RCS_ATTRIBUTES + errorIndex);
                return TPM_RCS_ATTRIBUTES + errorIndex;
            }
        ...
        if(s_associatedHandles[sessionIndex] != TPM_RH_UNASSIGNED)
        {
            ...
            if(result != TPM_RC_SUCCESS){
                LOG_INFO("AuthSes: %d Session Index: %d, associatedHandle: %08x", result, sessionIndex, s_associatedHandles[sessionIndex]) ;
        ...
        else
        {
            ...
            if(!IS_ATTRIBUTE(s_attributes[sessionIndex], TPMA_SESSION, audit)
               && !IS_ATTRIBUTE(s_attributes[sessionIndex], TPMA_SESSION, encrypt)
               && !IS_ATTRIBUTE(s_attributes[sessionIndex], TPMA_SESSION, decrypt)){
                LOG_INFO("Error Return: %08x",TPM_RCS_ATTRIBUTES + errorIndex );
                return TPM_RCS_ATTRIBUTES + errorIndex;
               }
