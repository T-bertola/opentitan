...
#include "TPM/TPMCmd/tpm/include/public/TpmTypes.h"
...
)
{
    ...
    INT32  size   = (INT32)inRequestSize;
    ...
    switch(header.code)
    {
        case TPM_CC_GetTestResult:
            ...
            if(header.size != 10)
                goto FailureModeReturn;
            buffer      = &response[10];
            marshalSize = (UINT32)MarshalUint16(3 * sizeof(UINT32), &buffer);
            marshalSize += (UINT32)MarshalUint32(s_failFunction, &buffer);
            marshalSize += (UINT32)MarshalUint32(s_failLine, &buffer);
            marshalSize += (UINT32)MarshalUint32(s_failCode, &buffer);
            if(s_failCode == FATAL_ERROR_NV_UNRECOVERABLE)
                marshalSize += (UINT32)MarshalUint32(TPM_RC_NV_UNINITIALIZED, &buffer);
            else
                marshalSize += (UINT32)MarshalUint32(TPM_RC_FAILURE, &buffer);
            break;
        case TPM_CC_GetCapability:
            ...
            marshalSize += (UINT32)MarshalUint32(capability, &buffer);
            // indicate the number of values that are being returned (0 or 1)
            marshalSize += (UINT32)MarshalUint32(count, &buffer);
            // indicate the property
            marshalSize += (UINT32)MarshalUint32(pt, &buffer);
...
FailureModeReturn:
    buffer      = response;
    marshalSize = (UINT32)MarshalUint16(TPM_ST_NO_SESSIONS, &buffer);
    marshalSize += (UINT32)MarshalUint32(10, &buffer);
    marshalSize += (UINT32)MarshalUint32(TPM_RC_FAILURE, &buffer);
    *outResponseSize = marshalSize;
    *outResponse     = (unsigned char*)response;
    return;
