...
#include "TPM/TPMCmd/tpm/include/private/CommandAttributeData.h"
...
{
    //Currently Can't figure out the authorization roles so I have elected to 
    //bypass them until I have time...
    //return AUTH_NONE;
    if(0 == handleIndex)
    {
        // Any authorization role set?
        COMMAND_ATTRIBUTES properties = s_commandAttributes[commandIndex];
        //LOG_INFO("Prop: %d, test1 %d, test2 %d, test3 %d", properties, properties & HANDLE_1_USER, properties & HANDLE_1_ADMIN, properties & HANDLE_1_DUP);
