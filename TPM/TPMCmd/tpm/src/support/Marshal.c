    ...
    TPMI_DH_OBJECT* target, BYTE** buffer, INT32* size, BOOL flag)
{
    ...
    //LOG_INFO("RESULT: %d, target %08x, flag %d", result, *target, flag);
    if((result == TPM_RC_SUCCESS) && ((*target != TPM_RH_NULL) || !flag)
       ...
       && ((*target < PERSISTENT_FIRST) || (*target > PERSISTENT_LAST)))
        result = TPM_RC_VALUE;
        //LOG_INFO("RESULT %d, Tran_fr %08x, Tran_ls %08x, per_fr %08x, per_lst %08x", result, TRANSIENT_FIRST, TRANSIENT_LAST, PERSISTENT_FIRST, PERSISTENT_LAST);
    return result;
}
#  if !USE_MARSHALING_DEFINES
UINT16
    ...
    TPMI_ALG_SYM_MODE* target, BYTE** buffer, INT32* size, BOOL flag)
{
    ...
    result = TPM_ALG_ID_Unmarshal((TPM_ALG_ID*)target, buffer, size);
    #ifdef DEBUG_PRINT
    LOG_INFO("Res: %d, Target: %d", result, *target);
    #endif
    if(result == TPM_RC_SUCCESS)
    {
        switch(*target)
...
TPMI_AES_KEY_BITS_Unmarshal(TPMI_AES_KEY_BITS* target, BYTE** buffer, INT32* size)
{
    ...
    #ifdef DEBUG_PRINT
    LOG_INFO("Result: %d, targ: %08x", result, *target);
    #endif
    if(result == TPM_RC_SUCCESS)
    {
        switch(*target)
    ...
    TPMT_SYM_DEF_OBJECT* target, BYTE** buffer, INT32* size, BOOL flag)
{
    ...
    if(result != TPM_RC_SUCCESS){
    LOG_INFO("TPMI_ALG_SYM_OBJECT_Unmarshal: %d", result);
    }
    if(result == TPM_RC_SUCCESS){
        result = TPMU_SYM_KEY_BITS_Unmarshal((TPMU_SYM_KEY_BITS*)&(target->keyBits),
                                             buffer,
                                             ...
                                             (UINT32)target->algorithm);
        if(result != TPM_RC_SUCCESS){
        LOG_INFO("TPMU_SYM_KEY_BITS_Unmarshal: %d", result);
        }
    }
    if(result == TPM_RC_SUCCESS){
        result = TPMU_SYM_MODE_Unmarshal(
            (TPMU_SYM_MODE*)&(target->mode), buffer, size, (UINT32)target->algorithm);
        if(result != TPM_RC_SUCCESS){
            LOG_INFO("TPMU_SYM_MODE_Unmarshal: %d", result);
        }
    }
    return result;
}
...
#  endif  // ALG_OAEP
        ...
        LOG_INFO("TPMI_ALG_RSA_DECRYPT_Unmarshal ERROR: Result = %d", result);
    }
    return result;
}
#  if !USE_MARSHALING_DEFINES
    ...
    TPMT_RSA_DECRYPT* target, BYTE** buffer, INT32* size, BOOL flag)
{
    ...
    if(result == TPM_RC_SUCCESS){
        ...
        LOG_INFO("ERROR TPMI_ALG_RSA_DECRYPT_Unmarshal: Result = %d", result);
    }
    return result;
}
UINT16
...
TPMS_RSA_PARMS_Unmarshal(TPMS_RSA_PARMS* target, BYTE** buffer, INT32* size)
{
    ...
    if(result != TPM_RC_SUCCESS){
    LOG_INFO("TPMT_SYM_DEF_OBJECT_Unmarshal: %d", result);
    }
    if(result == TPM_RC_SUCCESS){
        result = TPMT_RSA_SCHEME_Unmarshal(
            (TPMT_RSA_SCHEME*)&(target->scheme), buffer, size, 1);
        if(result != TPM_RC_SUCCESS){
            LOG_INFO("TPMT_RSA_SCHEME_Unmarshal: %d", result);
        }
    }
    if(result == TPM_RC_SUCCESS){
        ...
        if(result != TPM_RC_SUCCESS){
            LOG_INFO("TPMI_RSA_KEY_BITS_Unmarshal: %d", result);
        }
    }
    if(result == TPM_RC_SUCCESS){
        result = UINT32_Unmarshal((UINT32*)&(target->exponent), buffer, size);
        if(result != TPM_RC_SUCCESS){
            LOG_INFO("UINT32_Unmarshal: %d", result);
        }
    }
    return result;
}
...
TPMT_PUBLIC_Unmarshal(TPMT_PUBLIC* target, BYTE** buffer, INT32* size, BOOL flag)
{
    ...
    result =
        TPMI_ALG_PUBLIC_Unmarshal((TPMI_ALG_PUBLIC*)&(target->type), buffer, size);
    if(result != TPM_RC_SUCCESS){
    LOG_INFO("TPMI_ALG_PUBLIC_Unmarshal: %d", result);
    }
    if(result == TPM_RC_SUCCESS){
        result = TPMI_ALG_HASH_Unmarshal(
            (TPMI_ALG_HASH*)&(target->nameAlg), buffer, size, flag);
       if(result != TPM_RC_SUCCESS){
        LOG_INFO("TPMI_ALG_HASH_Unmarshal: %d", result);
        }
    }
    if(result == TPM_RC_SUCCESS){
        ...
        if(result != TPM_RC_SUCCESS){
        LOG_INFO("TPMA_OBJECT_Unmarshal: %d", result);
        }
    }
    if(result == TPM_RC_SUCCESS){
        ...
        if(result != TPM_RC_SUCCESS){
        LOG_INFO("TPM2B_DIGEST_Unmarshal: %d", result);
        }
    }
    if(result == TPM_RC_SUCCESS){
        result =
            TPMU_PUBLIC_PARMS_Unmarshal((TPMU_PUBLIC_PARMS*)&(target->parameters),
                                        ...
                                        (UINT32)target->type);
        if(result != TPM_RC_SUCCESS){
        LOG_INFO("TPMU_PUBLIC_PARMS_Unmarshal: %d", result);
        }
    }
    if(result == TPM_RC_SUCCESS){
        ...
        if(result != TPM_RC_SUCCESS){
        LOG_INFO("TPMU_PUBLIC_ID_Unmarshal: %d", result);
        }
    }
    return result;
}
...
TPM2B_PUBLIC_Unmarshal(TPM2B_PUBLIC* target, BYTE** buffer, INT32* size, BOOL flag)
{
    ...
    if(result == TPM_RC_SUCCESS)
    {
        ...
        else
        {
            ...
            if(result != TPM_RC_SUCCESS){
                LOG_INFO("Result: %d, targetSize %d, startSize, %d size %d", result, target->size, startSize, *size);
            }
            if((result == TPM_RC_SUCCESS) && (target->size != (startSize - *size)))
                result = TPM_RC_SIZE;
        }
