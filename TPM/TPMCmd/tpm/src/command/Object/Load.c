...
)
{
    ...
    if(in->inPrivate.t.size == 0){
        LOG_INFO("in->inPrivate.t.size == 0");
    ...
    if(!ObjectIsParent(parentObject)){
        LOG_INFO("!ObjectIsParent(parentObject) FAILED:parentObject = %08x",parentObject);
    ...
    if(result != TPM_RC_SUCCESS){
        LOG_INFO("PrivateToSensitive Failed");
    ...
    result = ObjectLoad(newObject,
                        ...
                        RC_Load_inPrivate,
                        &out->name);
    LOG_INFO("OBJECT RESULT: %d", result);
    if(result == TPM_RC_SUCCESS)
    {
