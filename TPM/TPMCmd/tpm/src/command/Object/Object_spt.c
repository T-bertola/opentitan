...
)
{
    ...
    if(result == TPM_RC_SUCCESS)
    {
        ...
        if(!MemoryEqual2B(&integrity.b, &integrityToCompare.b)){
            LOG_INFO("RC INTEG");
            for(int i = 0; i <integrity.b.size; i++ ){
                LOG_INFO("Integrity: %04x, CompIntegrity: %04x", integrity.b.buffer[i], integrityToCompare.b.buffer[i]);
            }
        ...
        if(useIV)
        {
            ...
            if(result == TPM_RC_SUCCESS)
            {
                ...
                LOG_INFO("TPM2B_IV_Unmarshal Failed: Result: %d", result);
            }
        }
    }
    else{
        LOG_INFO("TPM2B_DIGEST_Unmarshal Failed");
    }
...
)
{
    ...
    if(result != TPM_RC_SUCCESS){
        LOG_INFO("UnwrapOuter Failed: Result = %d",result);
        return result;
    }
    ...
    if(result == TPM_RC_SUCCESS)
    {
        if((dataSizeInput + sizeof(UINT16)) != dataSize){
            ...
            LOG_INFO("(dataSizeInput + sizeof(UINT16)) != dataSize");
            LOG_INFO("DataSizeInput = %d, sizeof(UINT16) = %d, datasize = %d ", dataSizeInput, (UINT32)sizeof(UINT16), dataSize);
        }
        else
        {
            ...
            {
                LOG_INFO("result != TPM_RC_SUCCESS || size != 0");
                result = TPM_RC_SENSITIVE;
            }
        }
    }
    else{
        LOG_INFO("UINT16_Unmarshal Failed");
    }
    return result;
}
