...
def gen_aes_key(serial_port):
    ...
    0x00, 0x09, 0x40, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 
    0x00, 0x00, 0x32, 0x00, 0x25, 0x00, 0x0b, 0x00, 0x06, 0x00, 0x32, 0x00, 0x20, 0x7d, 0xf0, 0x52, 
    0xf3, 0x68, 0x36, 0xe4, 0x21, 0x76, 0xaa, 0xa0, 0xb8, 0x00, 0xa5, 0x6e, 0xf3, 0x5e, 0xd7, 0x28,
    0x00, 0x00, 0x00
])  

    tpm_aes_key_gen_2 = bytearray([
    0x00,
    0xf4, 0x6c, 0x6c, 0xb4, 0xcc, 0x83, 0xd5, 0x60, 0x59, 0x49, 0x0b, 0xa3, 0x61, 0x00, 0x06, 0x00, 
...
def load_key(serial_port):
    ...
    private = bytearray([
    0x8e, 
    0x00, 0x20, 0x1d, 0xab, 0x9f, 0x26, 0x79, 0xf6, 0xcc, 0xff, 0xbf, 0xfe, 0x95, 0x65, 0x3f, 0xae, 
    0xc4, 0x9d, 0x68, 0x3c, 0x12, 0x18, 0x12, 0xb2, 0x03, 0xa7, 0x32, 0xca, 0xc8, 0x50, 0x83, 0xf0, 
    0xf2, 0x16, 0x00, 0x10, 0x4d, 0x98, 0xca, 0x70, 0xe8, 0x9a, 0x9a, 0xe8, 0x78, 0x43, 0xbb, 0x29, 
    ...
    private1 = bytearray([
    
    ])
    public = bytearray([
    ...
    public1 = bytearray([
    0x82, 0x38, 0x9d, 0x70, 0x21, 0x24, 0x46, 0x66, 0x33, 0xef, 0x47, 0x9d, 0xc1, 0x25, 0x45, 0x8e, 
    0xfb, 0x67, 0xa5, 0xcd, 0x3c, 0x5f, 0xc7, 0xdb, 0x37, 0x07, 0x9d, 0x04, 0x45, 0xe9, 0xde, 0xe5
    ])
    ...
    0x0b, 0x00, 0x06, 0x00, 0x32, 0x00, 0x20, 0x7d, 0xf0, 0x52, 0xf3, 0x68, 0x36, 0xe4, 0x21, 0x76, 
    0xaa, 0xa0, 0xb8, 0x00, 0xa5, 0x6e, 0xf3, 0x5e, 0xd7, 0x28, 0xf4, 0x6c, 0x6c, 0xb4, 0xcc, 0x83, 
    0xd5, 0x60, 0x59, 0x49, 0x0b, 0xa3, 0x61, 0x00, 0x06, 0x00, 0x80, 0x00, 0x43, 0x00, 0x20
    ])
    load_key_ary.extend(private1)
    load_key_ary.extend(load_key_ary_2)
    load_key_ary.extend(public1)
...
def decrypt_seal(serial_port):
    decrypt = bytearray([
    0x80, 0x02, 0x00, 0x00, 0x00, 0x37, 0x00, 0x00, 0x01, 0x64, 0x80, 0x00, 0x00, 0x01, 0x00, 0x00,
    0x00, 0x09, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x10, 0x00, 0x10, 
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x05, 0x7b, 0x1c, 0x3b, 0xa4, 0x19
    ])

    array_size= len(decrypt)
    new_array = adjust_list(decrypt)
    send_to_tpm(serial_port, new_array, array_size)
    input("Pres Enter when DECRYPT is done") 


def create_rsa_sign_key(serial_port):
    rsa_sign = bytearray([
    0x80, 0x02, 0x00, 0x00, 0x00, 0x61, 0x00, 0x00, 0x01, 0x31, 0x40, 0x00, 0x00, 0x01, 0x00, 0x00, 
    0x00, 0x09, 0x40, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 
    0x00, 0x00, 0x38, 0x00, 0x01, 0x00, 0x0b, 0x00, 0x05, 0x00, 0x72, 0x00, 0x00, 0x00, 0x10, 0x00,
    0x14, 0x00, 0x0b, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x20, 0xe3, 0xb0, 0xc4, 0x42, 0x98, 
    0xfc, 0x1c, 0x14, 0x9a, 0xfb, 0xf4, 0xc8, 0x99, 0x6f, 0xb9, 0x24, 0x27, 0xae, 0x41, 0xe4, 0x64, 
    0x9b, 0x93, 0x4c, 0xa4, 0x95, 0x99, 0x1b, 0x78, 0x52, 0xb8, 0x55, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00  
    ])
    blocks_of_data = [rsa_sign[i:i + 64] for i in range(0, len(rsa_sign), 64)]
    for i, block in enumerate(blocks_of_data):
        len_array = len(block)
        new_array = adjust_list(block)
        send_to_tpm(serial_port, new_array, len_array)
...
def quote_tpm(serial_port):
    quote = bytearray([
    0x80, 0x02, 0x00, 0x00, 0x00, 0x36, 0x00, 0x00, 0x01, 0x58, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 
    0x00, 0x09, 0x40, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0d, 0x30, 0x2e, 0x31,
    0x72, 0x37, 0x65, 0x70, 0x39, 0x39, 0x68, 0x37, 0x62, 0x70, 0x00, 0x10, 0x00, 0x00, 0x00, 0x01,
    0x00, 0x0b, 0x03, 0x0f, 0x00, 0x00    
    ])
    send_to_tpm(serial_port, adjust_list(quote), len(quote)-2)

def gen_rsa_bind_key(serial_port):
    rsa_key = bytearray([
    0x80, 0x02, 0x00, 0x00, 0x00, 0x5f, 0x00, 0x00, 0x01, 0x31, 0x40, 0x00, 0x00, 0x01, 0x00, 0x00,
    0x00, 0x09, 0x40, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x36, 0x00, 0x01, 0x00, 0x0b, 0x00, 0x06, 0x00, 0x72, 0x00, 0x00, 0x00, 0x10, 0x00,
    0x10, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x20, 0xe3, 0xb0, 0xc4, 0x42, 0x98, 0xfc, 0x1c, 
    0x14, 0x9a, 0xfb, 0xf4, 0xc8, 0x99, 0x6f, 0xb9, 0x24, 0x27, 0xae, 0x41, 0xe4, 0x64, 0x9b, 0x93, 
    0x4c, 0xa4, 0x95, 0x99, 0x1b, 0x78, 0x52, 0xb8, 0x55, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
    ])
    blocks_of_data = [rsa_key[i:i + 64] for i in range(0, len(rsa_key), 64)]
    for i, block in enumerate(blocks_of_data):
        len_array = len(block)
        new_array = adjust_list(block)
        send_to_tpm(serial_port, new_array, len_array)
        input("Press Enter when the block is read in")
    input("Enter when Key generated")

def encrypt_bind(serial_port):
    bind_encrypt = bytearray([
    0x80, 0x01, 0x00, 0x00, 0x00, 0x19, 0x00, 0x00, 0x01, 0x74, 0x80, 0x00, 0x00, 0x00, 0x00, 0x05, 
    0x11, 0x22, 0x33, 0x44, 0x55, 0x00, 0x15, 0x00, 0x00
    ])
    array_size= len(bind_encrypt)
    new_array = adjust_list(bind_encrypt)
    send_to_tpm(serial_port, new_array, array_size)
    input("Pres Enter when ENCRYPT is done") 

def decrypt_bind(serial_port):
    decrypt_lst = bytearray([
    

    ])

    blocks_of_data = [decrypt_lst[i:i + 64] for i in range(0, len(decrypt_lst), 64)]
    for i, block in enumerate(blocks_of_data):
        len_array = len(block)
        new_array = adjust_list(block)
        print(" ".join(f"{x:02x}" for x in decrypt_lst))
        print("SIZE", len_array)
        send_to_tpm(serial_port, new_array, len_array)
        input("Press Enter when the block is read in")
    input("Press Enter when LOAD is done")
        ...
        with serial.Serial(tpm_device, baudrate=115200, timeout=1) as serial_port:
            ...
            if (args.send):
                if (args.send.upper() == "RNG"):
                    ...
                    input("Enter when Encryption is done")
                    policy_pcr(serial_port)
                    decrypt_seal(serial_port)

                elif(args.send.upper() == "RA"):

                    start_tpm(serial_port)
                    input("Enter when START is done")
                    pcr_extend(serial_port)
                    input("Enter when EXTEND PCR is done")
                    create_rsa_sign_key(serial_port)
                    input("Enter when GEN SIGN RSA KEY is done")
                    quote_tpm(serial_port)
                    input("Enter when QUOTE is done") 

                elif(args.send.upper() == "BIND"):
                    start_tpm(serial_port)
                    input("Enter when START is done")
                    gen_rsa_bind_key(serial_port)
                    input("Enter when CREATE is done")
                    encrypt_bind(serial_port)
                    decrypt_bind(serial_port)
